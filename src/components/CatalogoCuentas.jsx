...
function manejarExportacionExcel() {
    if (!cuentasFiltradas.length) {
        setError("No hay cuentas para exportar.");
        return;
    }

    try {
        const filas = cuentasFiltradas.map(cuenta => {
            const padre = cuentasPorId.get(String(cuenta.cuenta_padre_id));

            return `
                <tr>
                    <td>${escaparExcel(cuenta.codigo)}</td>
                    <td>${escaparExcel(cuenta.nombre)}</td>
                    <td>${escaparExcel(cuenta.tipo)}</td>
                    <td>${escaparExcel(cuenta.naturaleza)}</td>
                    <td>${escaparExcel(cuenta.nivel)}</td>
                    <td>${escaparExcel(
                        padre
                            ? `${padre.codigo} - ${padre.nombre}`
                            : "Principal (Raíz)"
                    )}</td>
                    <td>${cuenta.permite_movimientos ? "Sí" : "No"}</td>
                    <td>${cuenta.tiene_movimientos ? "Sí" : "No"}</td>
        ...
        const html = `
            ...
            <body>
                ...
                <div class="subtitulo">
                    Exportación del catálogo contable
                </div>
        ...
        enlace.download = `catalogo-cuentas-${fecha}.xls`;
        ...
        setMensajeExito(
            `${cuentasFiltradas.length} cuenta(s) exportada(s) correctamente a Excel.`
        );
    ...
    title={
        cuentasFiltradas.length
            ? `Exportar ${cuentasFiltradas.length} cuentas a Excel`
    ...
    style={{
        ...
        background: !cuentasFiltradas.length ? "#EEF2EF" : "#F4FAF6",
        color: !cuentasFiltradas.length ? "#9AA49F" : "#1B4332",
        fontSize: "13px",
        fontWeight: "600",
        cursor: !cuentasFiltradas.length ? "not-allowed" : "pointer",
        transition: "all 150ms ease",
        opacity: !cuentasFiltradas.length ? 0.7 : 1
    }}
    onMouseEnter={e => {
        if (cuentasFiltradas.length) {
            e.currentTarget.style.background = "#E3F0E7";
            e.currentTarget.style.borderColor = "#8FBA9F";
        }
    }}
    onMouseLeave={e => {
        if (cuentasFiltradas.length) {
            e.currentTarget.style.background = "#F4FAF6";
            e.currentTarget.style.borderColor = "#B8D4C1";
        }
    }}
>
    <svg
