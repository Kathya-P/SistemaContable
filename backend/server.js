...
// ==========================================================
// Asientos Guardados (Esqueletos / Plantillas de Asientos)
// ==========================================================
let _asientosGuardadosEnMemoria = [];

apiRouter.get("/asientos-guardados", async (req, res) => {
    try {
        const usuario = await obtenerUsuarioAutenticado(req);
        const empresaId = Number(usuario.empresa_id);

        if (!getSupabaseUrl() || !getSupabaseKey()) {
            const lista = _asientosGuardadosEnMemoria
                .filter(item => Number(item.empresa_id) === empresaId)
        ...
        try {
            ...
            // tabla no existe en supabase, usar memoria
        }
...
apiRouter.post("/asientos-guardados", async (req, res) => {
    try {
        const usuario = await obtenerUsuarioAutenticado(req);
        await exigirPermiso(usuario, "puede_crear_asientos");

        const { concepto, modo_iva, detalles } = req.body || {};
        if (!concepto || !String(concepto).trim()) {
            throw new Error("El concepto/comentario del asiento guardado es obligatorio.");
        }
        if (!Array.isArray(detalles) || detalles.length < 2) {
            throw new Error("El esqueleto debe contener al menos 2 líneas de cuentas.");
        }
        ...
        const nuevoItem = {
            ...
            detalles: detalles.map(d => ({
                ...
                debe: d.debe ? Number(d.debe) : 0,
                haber: d.haber ? Number(d.haber) : 0
            })),
        ...
        if (getSupabaseUrl() && getSupabaseKey()) {
            try {
                ...
                if (!error && data) {
                    _asientosGuardadosEnMemoria.unshift(data);
                    return res.status(201).json({ ok: true, asiento: data });
                }
            } catch {
                // fallback en memoria
            }
        ...
        _asientosGuardadosEnMemoria.unshift(nuevoItem);
        return res.status(201).json({ ok: true, asiento: nuevoItem });
    } catch (error) {
        return responderError(res, error);
    }
});

apiRouter.delete("/asientos-guardados/:id", async (req, res) => {
    try {
        const usuario = await obtenerUsuarioAutenticado(req);
        const id = req.params.id;

        _asientosGuardadosEnMemoria = _asientosGuardadosEnMemoria.filter(
        ...
        return res.json({ ok: true, mensaje: "Asiento guardado eliminado." });
    } catch (error) {
        return responderError(res, error);
    }
});
